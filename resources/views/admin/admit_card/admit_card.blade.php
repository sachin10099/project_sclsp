    ...
    <div class="row">
    	<div class="col-sm-4">
    		<div class="content-header">
		        <h4>Manage Admit Cards</h4>
		    </div>
    	</div>
    	<div class="col-sm-8">
        <div class="btn-group pull-right">
          <a href="{{ url('form-filler/admit-card/add') }}"><button type="button" class="btn btn-info">Add Admit Cards</button></a>
        </div>
    	</div>
    </div>
    <br>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
      document.getElementById('loader').style.display = "none";
        ...
        processing: true,
        serverSide: true,
        ajax: '{!! url('form-filler/admit-card/data') !!}', 
        "aoColumnDefs": [ {
               "aTargets": [ 1 ],
  ...
  function changeStatus(id) {
    $.ajax({
          method:'post',
          url   : "{{ url('form-filler/admit-card/change-status') }}",
          data  : {
              "_token": "{{ csrf_token() }}",
              'id'    : id,
          },
